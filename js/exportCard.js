...
function detailSections(entry, locomotives) {
  ...
  const departure = [
    ...
    {...item("HOG Attached", entry.hogAttachedTime && entry.hogAttachedToTime ? time(entry.hogAttachedTime)+" TO "+time(entry.hogAttachedToTime) : entry.hogAttachedTime ? "FROM "+time(entry.hogAttachedTime) : entry.hogAttachedToTime ? "TO "+time(entry.hogAttachedToTime) : ""), pairKey:"hog-time-place"},
    {...item("HOG Attached Place", entry.hogAttachedPlace), pairKey:"hog-time-place"},
